...
import {
	WALLPAPER_SOURCE_FALLBACK_EVENT,
	applyWallpaperSource,
	getWallpaperSource,
	setWallpaperSource,
} from "@/scripts/wallpaper-source.js";
...
// 壁纸图源：本地图片 / 二次元图源 / Bing 每日
const WALLPAPER_SOURCE_OPTIONS = [
	{ id: "local", label: "本地图片", icon: "material-symbols:folder-outline" },
	{ id: "scenery", label: "二次元风景", icon: "material-symbols:landscape-outline" },
	{ id: "character", label: "二次元立绘", icon: "material-symbols:person-outline" },
	{ id: "bing", label: "Bing 每日", icon: "material-symbols:today-outline" },
];

let wallpaperSource = $state("local");

function switchWallpaperSource(source: string) {
	wallpaperSource = source;
	setWallpaperSource(source);
	// reseed：每次点击都换一张新图，而不是复用上一次的地址
	applyWallpaperSource(source, { reseed: true });
}
...
onMount(() => {
	...
	const handleWallpaperSourceFallback = () => {
		wallpaperSource = "local";
	};
	document.addEventListener(
		WALLPAPER_SOURCE_FALLBACK_EVENT,
		handleWallpaperSourceFallback,
	);
...
<div
	...
	class="float-panel float-panel-closed absolute transition-all w-80 right-4 px-4 py-2
		max-h-[calc(100vh-5.5rem)] overflow-y-auto overscroll-contain settings-panel-scroll"
		...
		<div class="mt-2 mb-2">
			...
			>
				...
				<button
					aria-label="Reset to Default"
					class="btn-regular w-7 h-7 rounded-md active:scale-90"
					class:opacity-0={wallpaperSource === "local"}
					class:pointer-events-none={wallpaperSource === "local"}
					onclick={() => switchWallpaperSource("local")}
				>
					<div class="text-(--btn-content)">
						<Icon icon="material-symbols:refresh" class="text-[0.875rem]" />
					</div>
			...
			<div class="space-y-1">
				{#each WALLPAPER_SOURCE_OPTIONS as option (option.id)}
					<button
						class="w-full btn-regular rounded-md py-2 px-3 flex items-center gap-3 text-left active:scale-95 transition-all relative overflow-hidden"
						class:opacity-60={wallpaperSource !== option.id}
						class:bg-(--btn-regular-bg-hover)={wallpaperSource === option.id}
						onclick={() => switchWallpaperSource(option.id)}
					>
						<Icon icon={option.icon} class="text-[1.25rem] shrink-0" />
						<span class="text-sm flex-1">{option.label}</span>
						{#if wallpaperSource === option.id}
							<Icon icon="material-symbols:check-circle" class="text-[1rem] shrink-0 text-(--primary)" />
						{/if}
					</button>
				{/each}
			</div>
			<p class="text-xs text-neutral-500 dark:text-neutral-400 mt-2 px-1 leading-relaxed">
				远程图源来自第三方公开接口，内容与可用性不受本站控制。
			</p>
		</div>
	{/if}
