 ...
 * Pio 看板娘配置
 */
export interface DailyPigConfig {
	enable: boolean; // 是否启用今日小猪
	data: string; // 小猪数据 JSON 路径
	imageBase: string; // 小猪图片目录
	title: string; // 弹窗标题
	loading: string; // 抽取中提示
...
export interface PioConfig {
	...
	animation?: {
		image: string; // 点击时播放的动画图片路径
		duration: number; // 动画一轮播放时长（ms），需与素材实际时长一致
	};
